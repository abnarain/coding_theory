...
cdwrd_est = decode_uncoded_qpsk(rcvd_dem,col_); % decodes the demodulated bit sequence to form an estimate cdwrd_est
ber = sum(msg~=cdwrd_est)/n % computes the BER over the codeword
global_ber=[global_ber,ber];
end

theor_BER_uncodedQpsk = qfunc(sqrt(2*Eb_by_N0));
semilogy(Eb_by_N0_db,global_ber,'b+',Eb_by_N0_db,theor_BER_uncodedQpsk,'r*');
...
%save('/tmp/save_ber_uncoded_qpsk','ber');
%save('/tmp/save_Eb_by_N0_db_uncoded_qpsk','Eb_by_N0_db');
